		...
		while (pThunk->u1.Function) {
			if (DWORD(pThunk->u1.Function) == dwAddressToIntercept) {	
				if (
					...
					pThunk->u1.Function = (unsigned long)pApiNew;
					bSuccess = TRUE;
				}
				else {
					if (
	...
	for (;; imports++, thunks++ )
    {
         ...
         else
		{
		 ...
		 if ( !IMAGE_SNAP_BY_ORDINAL (test_ordinal) )
		 {
			 ...
			 {
			 if (oldaddr) oldaddr = (unsigned long*)thunks->u1.Function; 
			 if (newaddr) 
			 {
				 if (!IsBadWritePtr((LPVOID)(&thunks->u1.Function), sizeof(DWORD)))
				 {
					thunks->u1.Function = (unsigned long)&newaddr;
				 }
				 else
				 {
				 DWORD dwProtect,dwNewProtect;
